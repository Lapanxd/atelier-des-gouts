...
const ImageSetCard = (props) => {
  ...
  return (
    <>
      {image ? (
        <div
          className={`w-full h-20 flex flex-none gap-3 pl-2 bg-base-200 rounded-3xl hover:bg-base-300 cursor-pointer border-2 ${clicked ? 'border-solid border-2 border-blue-800' : null}`}
          onClick={onClickCard}
        >
          <img
            src={image}
